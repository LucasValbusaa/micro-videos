...
describe("CastMemberSequelizeRepository Unit Tests", () => {
  ...
  it("should delete a entity", async () => {
    ...
    const entityFound = await CastMemberModel.findByPk(entity.id.value);
  ...
  //       expect(searchOutput.toJSON().items[index]).toBeInstanceOf(CastMember);
  //       expect(item.toJSON()).toStrictEqual(
  //         searchOutput.toJSON().items[index].toJSON()
  //       );
  //     });
  //   });
  ...
  //       const { items, filter, ...otherOutput } = searchOutput.toJSON();
